...
app.get("/", (req, res) => {
  res.send("Servicio de WhatsApp funcionando");
});
...
app.post(
  "/api/whatsapp/request-qr",
  generalLimiter,
  whatsappController.requestQR,
);
...
app.post(
  ...
  sendImageLimiter,
  whatsappController.sendProductInfo,
);

// se envia un mensaje de bienvenida
app.post(
  "/api/whatsapp/send-welcome",
  generalLimiter,
  whatsappController.sendWelcome,
);
...
app.post(
  "/api/whatsapp/reset",
  generalLimiter,
  whatsappController.resetSession,
);
...
app.post(
  ...
  sendImageLimiter,
  whatsappController.sendCampaign,
);
...
io.on("connection", (socket) => {
  if (activeSocketConnections >= MAX_SOCKET_CONNECTIONS) {
    socket.emit("error", {
      message: "Servidor con demasiadas conexiones, intenta más tarde",
    });
