...
export async function sendProductInfo(req, res) {
  try {
    ...
    const { productName, description, phone, email, imageData, productoId } =
      req.body;
    ...
    try {
      ...
      if (productoId) {
        ...
        if (productRows.length > 0) {
          ...
          logger.info(
            `Usando plantilla personalizada para producto ID: ${productoId}`,
          );
        }
      }
      ...
      if (!templateText) {
        ...
        logger.info(
          "Usando plantilla por defecto (no hay plantilla personalizada)",
        );
      }
      ...
      logger.error("Error al obtener template de DB", {
 ...
 */
export async function sendWelcome(req, res) {
  try {
    if (!whatsappService.isReady) {
      return res.status(400).json({
        success: false,
    ...
    const welcomeMessage = getWelcomeTemplate({ name });

    const result = await whatsappService.sendText(jid, welcomeMessage);
    ...
    res.status(500).json({
      success: false,
      message: "Error desconocido al enviar el mensaje de bienvenida.",
    });
  }
}
  ...
  try {
    ...
    if (image) {
      ...
      try {
        ...
        logger.error("Error al descargar imagen de campaña", {
          error: error.message,
          image,
        });
        return res.status(400).json({
