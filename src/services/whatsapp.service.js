...
class WhatsAppService {
      ...
      if (shouldReconnect) {
        ...
        if (fs.existsSync(WHATSAPP_CONFIG.authPath)) {
          try {
            ...
            logger.warn(
              "Error al eliminar contenido de auth_info, reintentando...",
              { error: error.message },
            );
  ...
  async validateNumber(numberId) {
    ...
    try {
      ...
      logger.error("Error al validar número", {
        error: error.message,
        numberId,
      });
      return null;
  ...
  async sendText(jid, text) {
    ...
    try {
      ...
      logger.info(`Mnesaje de texto enviado: ${jid}`);
      return {
        success: true,
        messageId: result.key.id,
        chatId: jid,
      ...
      logger.error(
        `Error al enviar mensaje de texto: ${error.message}, ${jid}`,
      );
      throw error;
    }
  }
  ...
  async resetSession() {
    try {
      ...
      try {
        ...
        logger.error("Error al reinicializar después de fallo", {
